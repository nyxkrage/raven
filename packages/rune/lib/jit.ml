...
module Device = struct
  type t = Tolk of Tolk.Device.t | Pjrt of Rune_pjrt.Device.t

  let tolk device = Tolk device
  let pjrt device = Pjrt device
end
...
let trace_tolk (type a b c d) ?(device : Tolk.Device.t option)
...
let trace_pjrt (type a b c d) (device : Rune_pjrt.Device.t)
    (f : (a, b) Nx.t -> (c, d) Nx.t) : (a, b) Nx.t -> (c, d) Nx.t =
  Rune_pjrt.jit
    ~backend:(Rune_pjrt.Device.backend device)
    ~device_id:(Rune_pjrt.Device.device_id device)
    f

let trace ?device f =
  match device with
  | None -> trace_tolk f
  | Some (Device.Tolk device) -> trace_tolk ~device f
  | Some (Device.Pjrt device) -> trace_pjrt device f
...
let trace_graph (type a b c d) ~(device : Device.t)
    (f : (a, b) Nx.t -> (c, d) Nx.t) (x : (a, b) Nx.t) : traced =
  let device =
    match device with
    | Device.Tolk device -> device
    | Device.Pjrt _ ->
        invalid_arg "Rune.trace_graph: PJRT devices do not expose Tolk graphs"
  in
